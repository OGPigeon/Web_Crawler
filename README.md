...
An async, distributed web crawler built on **aiohttp** and **Redis**. It supports domain-scoped crawls, robots.txt compliance, per-domain rate limiting, and resumable runs via a Redis-backed bloom filter.
...
- Python 3.10+
- Redis running locally (or any accessible Redis URL)
...
python examples/python_docs.py
...
Set `bloom_persist=True` (the default). The bloom filter and queue are stored in Redis. Stop the crawler with `Ctrl+C` at any time and re-run the same script — already-visited URLs are skipped automatically.

To start fresh, set `bloom_persist=False` for one run (this clears the Redis bloom key and queue on startup).

---

## Storage Options

Three built-in options in `webcrawler/writers.py`:
